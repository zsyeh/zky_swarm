...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    {
      waypoint_sub_ = nh.subscribe("/move_base_simple/goal_" + std::to_string(planner_manager_->pp_.drone_id), 1, &EGOReplanFSM::waypointCallback, this);
    }
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    visualization_->displayGoalPoint(next_wp, Eigen::Vector4d(0, 0.5, 0.5, 1), 0.3, 0);//终点显示
  ...
  void EGOReplanFSM::waypointCallback(const geometry_msgs::PoseStampedPtr &msg)
  {
    ...
    Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z); //终点的高度 by chen
  ...
  {
    odom_pos_(0) = msg->pose.pose.position.x + offset_x;
    odom_pos_(1) = msg->pose.pose.position.y + offset_y;
    odom_pos_(2) = msg->pose.pose.position.z + offset_z;
