...
struct MappingData {
  ...
  std::vector<char> occupancy_buffer_inflate_cxr_;
...
private:
  ...
  ros::Publisher map_pub_, map_inf_pub_, map_all_pub_;
...
inline bool GridMap::isKnownFree(const Eigen::Vector3i& id) {
  ...
  return md_.occupancy_buffer_[adr] >= mp_.clamp_min_log_ && md_.occupancy_buffer_inflate_[adr] == 0 && md_.occupancy_buffer_inflate_cxr_[adr] == 0;
}
...
inline bool GridMap::isKnownOccupied(const Eigen::Vector3i& id) {
  ...
  return md_.occupancy_buffer_inflate_[adr] == 1 && md_.occupancy_buffer_inflate_cxr_[adr] == 1;
}
...
inline void GridMap::setOccupied(Eigen::Vector3d pos) {
  ...
  md_.occupancy_buffer_inflate_cxr_[id(0) * mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2) +
                                id(1) * mp_.map_voxel_num_(2) + id(2)] = 1;
}
...
inline int GridMap::getOccupancy(Eigen::Vector3d pos) {
  ...
  //by chen
  //if(pos(2) >= mp_.virtual_ceil_yp_) return 1;
